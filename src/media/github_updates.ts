...
import { trackedTreeFile, listReleaseJsonAssets, fetchContentVersion, fetchSourceFileText, parseRepo, type GitHubFile } from "./github";
...
export async function installGitHubUpdate(id: string, update: GitHubUpdate, mode: "merge" | "overwrite") {
  ...
  const text = await fetchSourceFileText(source, update.file);
  if (fingerprint(Storage.getAdventureById(id)?.githubSource) !== update.source) throw new Error("This set changed. Close this dialog and reopen the picker to check again.");
